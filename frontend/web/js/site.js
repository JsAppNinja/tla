(function ($) {
    $(function () {
        ...
        var loginFornVisible = false;

        //$(document).on("scroll", onScroll);

        $(".active").click(function () {
            $(".slide_up_login").slideDown('slow');
            loginFornVisible = true;
        });
        $(".close_btn").click(function () {
            $(".slide_up_login").slideUp('slow');
            loginFornVisible = false;
        });
        $(".b-start-box .b-button").click(function () {
            ...
            $(".active_try").slideToggle('slow');
        });
        $(".button_to_top").click(function () {
            $("html, body").animate({scrollTop: 0}, "slow");
        });

        $('.head-menu a.linked').on('click', function (e) {
            e.preventDefault();
            $('.head-menu li').removeClass('active');
            $(this).parent().addClass('active');
            var offset = loginFornVisible ? -260 : -150;
            $.scrollTo($(this).attr('href'), 500, {offset: {top: offset}});
        });

        $('.container_terms button').on('click', function () {
